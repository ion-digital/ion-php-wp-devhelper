...
class AdminNavMenuEditWalker extends Walker_Nav_Menu_Edit {
    ...
    private const OPENING_ELEMENT = '<p class="field-description description description-wide">';
    private const CLOSING_ELEMENT = '</p>';
    private const REPLACEMENT_ELEMENT = '<p class="field-description description description-wide __wp-devhelper-processed">';
                    ...
                    ?>
                        <p class="field-<?php echo($field['name']); ?> description description-wide">
                            <label for="edit-menu-item-<?php echo($field['name']); ?>-<?php echo $item_id; ?>">
                                <?php _e($field['label']); ?><br />
                                <?php // var_dump($item->meta); ?>
                                <?php echo $field['html'](esc_attr($item->meta[$field['name']]), null, null, "menu-item-{$field['name']}[{$item_id}]", "edit-menu-item-{$field['id']}-{$item_id}"); ?>
                            </label>
                        </p>
                    <?php    
                }
