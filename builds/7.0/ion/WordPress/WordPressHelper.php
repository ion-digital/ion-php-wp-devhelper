...
final class WordPressHelper implements IWordPressHelper
{
    ...
    private static function registerWrapperAction(string $actionName, callable $init, int $priority = 0, bool $returnFirstResult = false)
    {
    ...
    private static function initializeHelper(IHelperContext $context, array $wpHelperSettings, string $helperDir = null)
    {
        if (static::$helperInitialized === false) {
            ...
            $helperDirs = [];
            if ($helperDir === null) {
                $helperDirs = ['..' . DIRECTORY_SEPARATOR . '..' . DIRECTORY_SEPARATOR . '..' . DIRECTORY_SEPARATOR . '..', '..' . DIRECTORY_SEPARATOR . '..' . DIRECTORY_SEPARATOR . '..', '..' . DIRECTORY_SEPARATOR . '..', '..', '.', '..' . DIRECTORY_SEPARATOR . 'vendor' . DIRECTORY_SEPARATOR . 'wp-helper', '..' . DIRECTORY_SEPARATOR . 'includes' . DIRECTORY_SEPARATOR . 'wp-helper', '..' . DIRECTORY_SEPARATOR . 'include' . DIRECTORY_SEPARATOR . 'wp-helper', '..' . DIRECTORY_SEPARATOR . 'includes', 'vendor' . DIRECTORY_SEPARATOR . 'wp-helper', 'includes' . DIRECTORY_SEPARATOR . 'wp-helper', 'include' . DIRECTORY_SEPARATOR . 'wp-helper', 'includes', 'include'];
            ...
            static::addAction("template_redirect", function ($template) {
                if (is_404() && !PHP::toBool(PHP::filterInput('wp-helper-disable-quick-404', [INPUT_GET], FILTER_DEFAULT))) {
                    $wpHelperPath = Constants::HELPER_SITE;
                    $wpHelperSettingsPath = static::getAdminUrl('admin', 'wp-helper-settings');
                    $req = PHP::getServerRequestUri();
                    $unblockedUri = $req . (strpos($req, '?') ? '&' : '?') . "wp-helper-disable-quick-404=true";
                    wp_die("This is a replacement 404 page generated by <a target=\"_blank\" href=\"{$wpHelperPath}\">WP Helper</a> <br /><br /> To disable: either set <strong>WP_DEBUG</strong> to <em>false</em> or <a target=\"_blank\" href=\"{$wpHelperSettingsPath}\">go to the settings page</a>. <br /><br /> To see the original template, please <a href=\"{$unblockedUri}\">click here</a>.", "404 Not Found", ['response' => 404, 'exit' => true]);
                    return;
                }
                return $template;
            });
